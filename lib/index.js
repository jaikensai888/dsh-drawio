...
function webappSourceFor(version, sha256, expectedBytes = DRAWIO_WAR_BYTES) {
	return {
		version,
		sha256,
...
const DEFAULT_WEBAPP_SOURCE = webappSourceFor(DRAWIO_RELEASE_TAG, DRAWIO_WAR_SHA256);
/**
	...
	*/
	...
	async #install() {
		...
		try {
			...
			if (digest !== this.#source.sha256) throw new Error(`资源包校验失败：期望 sha256 ${this.#source.sha256}，实际 ${digest}`);
...
const Config = z.object({
	...
	autosaveDelayMs: z.number().min(200).max(1e4).default(DEFAULT_DRAWIO_CONFIG.autosaveDelayMs),
	writeDebounceMs: z.number().min(0).max(1e4).default(DEFAULT_DRAWIO_CONFIG.writeDebounceMs),
...
*/
function normalizeDiagramsDir(value) {
	if (typeof value !== "string") return DEFAULT_DIAGRAMS_DIR$1;
	const trimmed = value.trim().replace(/^[.][\\/]/u, "");
	if (trimmed === "" || isAbsolute(trimmed) || /^[A-Za-z]:/u.test(trimmed)) return DEFAULT_DIAGRAMS_DIR$1;
	const segments = normalize(trimmed).replace(/\\/gu, "/").split("/");
	if (segments.some((segment) => segment === ".." || segment === "")) return DEFAULT_DIAGRAMS_DIR$1;
...
*/
function resolveDrawioConfig(value) {
	...
	return {
		...
		autosaveDelayMs: asNumber(record["autosaveDelayMs"], DEFAULT_DRAWIO_CONFIG.autosaveDelayMs, 200, 1e4),
		writeDebounceMs: asNumber(record["writeDebounceMs"], DEFAULT_DRAWIO_CONFIG.writeDebounceMs, 0, 1e4),
		uiTheme: DRAWIO_UI_THEMES.includes(theme) ? theme : DEFAULT_DRAWIO_CONFIG.uiTheme,
...
function clientConfigOf(config) {
	return {
...
*/
function resolveWithinBase(base, candidate, label = "path", options = {}) {
	const normalizedBase = resolve(base);
	const target = resolve(normalize(candidate));
	if (options.allowOutside !== true && !isWithin(normalizedBase, target)) throw new DrawioError("forbidden", `${label} 越出当前工作区：${candidate}`, 403);
	return target;
}
/**
...
*/
function resolveRequestPath(base, raw, label = "path", options = {}) {
	...
	if (options.allowOutside !== true && !isWithin(normalizedBase, candidate)) throw new DrawioError("forbidden", `${label} 越出当前工作区：${value}`, 403);
	return candidate;
}
...
function diagramsDirOf(options) {
	return options.diagramsDir === void 0 || options.diagramsDir === "" ? DEFAULT_DIAGRAMS_DIR : options.diagramsDir;
}
...
async function writeBlank(cwd, target) {
	try {
...
async function createDiagram(options) {
	if (options.path !== void 0 && options.path.trim() !== "") {
		const target$1 = resolveRequestPath(options.cwd, options.path, "path", fenceOf(options));
		const existing = await statOrUndefined(target$1);
		if (existing !== void 0) throw new DrawioError("conflict", `文件已存在，未覆盖：${relativeToBase(options.cwd, target$1)}`, 409, {
			reason: "exists",
			currentMtimeMs: Math.round(existing.mtimeMs)
		});
		return await writeBlank(options.cwd, target$1);
	}
	...
	return await writeBlank(options.cwd, target);
}
...
async function diagramExists(options) {
	const info = await statOrUndefined(resolveRequestPath(options.cwd, options.path, "path", fenceOf(options)));
...
*/
async function scopeFor(deps, body) {
	return {
		cwd: (await deps.resolveWorkspace(requireSessionId(body), optionalString(body, "cwd"))).cwd,
...
function handleConfig(request, response, deps) {
	if (request.method !== "GET" && request.method !== "HEAD") throw new DrawioError("method-not-allowed", "GET /drawio/api/config 只接受 GET/HEAD", 405);
	writeOk(response, clientConfigOf(deps.config));
}
async function handleWorkspace(request, response, deps) {
	const body = await requirePostBody(request, "POST /drawio/api/workspace");
	writeOk(response, {
		...await deps.resolveWorkspace(requireSessionId(body), optionalString(body, "cwd")),
		diagramsDir: deps.config.diagramsDir
	});
}
...
async function handleCreate(request, response, deps) {
	...
	const options = await scopeFor(deps, body);
...
async function resolveWorkspaceInfo(options) {
	const canonicalCwd = await canonicalizePath(options.cwd);
	let entity;
	if (options.registry !== void 0) try {
	...
	const base = {
		...
		registered: entity !== void 0
	};
	if (entity === void 0) return base;
	return {
		...base,
		workspaceId: entity.id,
		...entity.title === void 0 ? {} : { workspaceTitle: entity.title }
	};
}
...
function apply(ctx, config) {
	...
	const installer = new WebappInstaller({ source: webappSourceFor(resolved.drawioVersion, resolved.drawioSha256) });
	const trustedHosts = () => {
		try {
	...
	const resolveWorkspace = async (sessionId, clientCwd) => {
		let cwd;
		try {
...
//#endregion
export { Config, apply, inject, name, sessionCwdOf };
